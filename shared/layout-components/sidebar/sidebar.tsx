...
const Sidebar = () => {
	...
	function Onhover() {
		if ((themeState.dataToggled == 'icon-overlay-close' || themeState.dataToggled == 'detached-close') && themeState.iconOverlay != 'open') {
	...
	function menuClose() {
		...
		if (themeState.dataNavLayout == "horizontal" || themeState.dataNavStyle == "menu-click" || themeState.dataNavStyle == "icon-click") {
			closeMenu();
		}
	}
	...
	function menuResizeFn() {
		...
		if (WindowPreSize.length > 1) {
			...
			if (currentWidth >= 992 && prevWidth < 992) {
				setTheme({ dataToggled: themeState.dataVerticalStyle === "doublemenu" ? "double-menu-open" : "" });
			}
		}
	}
	...
	function setSubmenu(event: any, targetObject: any, MenuItems = menuitems) {
		if ((window.screen.availWidth <= 992 || themeState.dataNavStyle != "icon-hover") && (window.screen.availWidth <= 992 || themeState.dataNavStyle != "menu-hover")) {
		if (!event?.ctrlKey) {
	...
	function toggleSidemenu(event: any, targetObject: any, MenuItems = menuitems) {
		...
		if ((themeState.dataNavStyle != "icon-hover" && themeState.dataNavStyle != "menu-hover") || (window.innerWidth < 992) || (themeState.dataNavLayout != "horizontal") && (themeState.dataToggled != "icon-hover-closed" && themeState.dataToggled != "menu-hover-closed")) {
			for (const item of MenuItems) {
				if (item === targetObject) {
					if (themeState.dataVerticalStyle == 'doublemenu' && item.active) { return; }
			...
			if (targetObject?.children && targetObject.active) {
				if (themeState.dataVerticalStyle == 'doublemenu' && themeState.dataToggled != 'double-menu-open') {
					setTheme({ dataToggled: "double-menu-open" });
				}
			}
			if (element && themeState.dataNavLayout == 'horizontal' && (themeState.dataNavStyle == 'menu-click' || themeState.dataNavStyle == 'icon-click')) {
	...
	function HoverToggleInnerMenuFn(event: any, item: any) {
		let element = event.target;
		if (element && themeState.dataNavLayout == "horizontal" && (themeState.dataNavStyle == "menu-hover" || themeState.dataNavStyle == "icon-hover")) {
			const listItem = element.closest("li");
			if (listItem) {
    ...
    <Link href={levelone.path + "/"} className={`side-menu__item ${levelone.selected ? 'active' : ''}`}>
        ...
        {themeState.dataVerticalStyle !== "doublemenu" ? levelone.icon : ""}
